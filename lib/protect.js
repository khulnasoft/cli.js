...
var debugProtect = require('debug')('khulnasoft:protect');
var Promise = require('es6-promise').Promise; // jshint ignore:line
var moduleToObject = require('khulnasoft-module');
...
// given an ignore ruleset (parsed from the .khulnasoft yaml file) and a array of
...
function filterIgnored(ignore, vuln) {
  ...
  return vuln.map(function (vuln) {
    ...
    // this is a cursory test to ensure that we're working with a khulnasoft format
    ...
    if (!Array.isArray(ignore[vuln.id])) {
      var error = new Error('old, unsupported .khulnasoft format detected');
...
function filterPatched(patched, vuln, cwd) {
  ...
  return vuln.map(function (vuln) {
    ...
    var res = filtered.some(function (vuln) {
      ...
      var flag = path.resolve(source, '.khulnasoft-' + vuln.id + '.flag');
      var res = false;
      try {
        res = statSync(flag);
      } catch (e) {}
  ...
  return new Promise(function (resolve) {
    ...
    var promises = _.flatten(packages.map(function (vuln) {
      ...
      var promises = patches.urls.map(function (url) {
        return new Promise(function (resolve, reject) {
          var filename = tempfile('.khulnasoft-patch');
