...
function test(path, options) {
  ...
  return khulnasoft.test(path || process.cwd(), options).then(function (res) {
    if (options.json) {
    ...
    if (res.ok) {
      ...
      summary += '\n\nNext steps:\n- Run `khulnasoft monitor` to be notified about' +
        ' new related vulnerabilities.\n- Run `khulnasoft test` as part of your ' +
        'CI/test.';
      return summary;
    }
          ...
          if (elem) {
            ...
            if (vuln.from.length > idx && vuln.from[idx] === elem) {
              ...
              fix += 'Your dependencies are out of date, otherwise you would ' +
                ...
                'and running `khulnasoft test` again.\nIf the problem persists, one' +
            ...
            if (idx === 0) {
              ...
              res += 'No direct dependency upgrade can address this issue.\n' +
                chalk.bold('Run `khulnasoft protect -i` to patch this vulnerability' +
